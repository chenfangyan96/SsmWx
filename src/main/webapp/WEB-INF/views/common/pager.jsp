...
<pg:pager maxPageItems="${page_size }" items="${param.items }" export="curPage=pageNumber" url="${param.url }">

<c:forEach items="${param.params }" var="p">
	<pg:param name="${p }" value="${p }"/>
</c:forEach>
<div class="page mt10"><div class="pagination"><ul>
<!-- 页码描述 -->
<pg:last>
...
<pg:first>
	...
	<c:if test="${curPage ne pageNumber }">
		<li class="first-child"><a href="#" onclick="goPager('${pageUrl}');">首页</a></li>
	</c:if>
</pg:first>

<!-- 上一页 -->
<pg:prev>
	<li><a href="#" onclick="goPager('${pageUrl}');">上一页</a></li>
</pg:prev>
...
<c:if test="${curPage ne pageNumber }">
	<li><a href="#" onclick="goPager('${pageUrl}');">${pageNumber }</a></li>
</c:if>
</pg:pages>

<!-- 下一页 -->
<pg:next>
	<li><a href="#" onclick="goPager('${pageUrl}');">下一页</a></li>
</pg:next>
...
<pg:last>
	...
	<c:if test="${curPage ne pageNumber && pageNumber ne 0}">
		<li class="last-child"><a href="#" onclick="goPager('${pageUrl}');">末页</a></li>
	</c:if>
</pg:last>
...
function goPager(pageUrl){
	var action = $("#searchForm").attr("action") + pageUrl;
	$("#searchForm").attr('action', action);
	$('#searchForm').submit();
}

function refresh(){
	var p_ = "?pager.offset=${pager.pageOffset*page_size}";
	var action = $("#searchForm").attr("action") + p_;
	$("#searchForm").attr('action', action);
	$('#searchForm').submit();
}
</script>